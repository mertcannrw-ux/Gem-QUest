  ...
  vm.runInContext(`
    function makeEnvGame() {
      ...
      Object.assign(g, {
        ...
        cam: { x: 0, y: 0 },
        vw: 1280, vh: 720,
        environmentStageId: null,
        environmentObjects: new Map(),
        environmentCells: new Set(),
        environmentSpatialHash: new Map(),
        environmentState: new Map(),
        environmentRenderBuffer: [],
        environmentCellSize: 128,
...
test('environment generation is deterministic for a stage and region', () => {
  ...
  const result = run(ctx, `(function () {
    ...
    g1.ensureEnvironmentAround(stage, 0, 0, 2);
    const ids1 = [...g1.environmentObjects.keys()].sort();
    const g2 = makeEnvGame();
    g2.ensureEnvironmentAround(stage, 0, 0, 2);
    const ids2 = [...g2.environmentObjects.keys()].sort();
    const firstTree = [...g1.environmentObjects.values()].find((o) => o.kind === 'tree');
    return {
      ids1, ids2,
      count: ids1.length,
...
test('two stages generate independent layouts', () => {
  ...
  const result = run(ctx, `(function () {
    ...
    forest.ensureEnvironmentAround({ id: 'forest', index: 0 }, 0, 0, 2);
    const forestIds = JSON.stringify([...forest.environmentObjects.keys()].sort());
    const caves = makeEnvGame();
    caves.ensureEnvironmentAround({ id: 'caves', index: 1 }, 0, 0, 2);
    const caveIds = JSON.stringify([...caves.environmentObjects.keys()].sort());
...
test('spatial hash inserts and removes environment objects', () => {
  ...
  const result = run(ctx, `(function () {
    ...
    g.addEnvironmentObject(obj);
    const key = g.environmentHashKey(200, 64);
    const bucket = g.environmentSpatialHash.get(key);
    const inserted = !!bucket && bucket.has('rock1');
    // Simulate pruning removal (mirrors pruneEnvironment bookkeeping).
    g.environmentObjects.delete('rock1');
    bucket.delete('rock1');
    const removed = !bucket.has('rock1') && !g.environmentObjects.has('rock1');
...
test('solid trees block actor movement', () => {
  ...
  const result = run(ctx, `(function () {
    ...
    g.addEnvironmentObject(tree);
    const actor = { x: -80, y: 0 };
    g.moveActorWithEnvironment(actor, 120, 0, 14);
...
test('traceEnvironmentHit returns the nearest solid object on a ray', () => {
  ...
  const result = run(ctx, `(function () {
    ...
    g.addEnvironmentObject(near);
    g.addEnvironmentObject(far);
    const hit = g.traceEnvironmentHit(-200, 0, 200, 0, 3);
...
test('non-solid props do not block traced projectiles', () => {
  ...
  const result = run(ctx, `(function () {
    ...
    g.addEnvironmentObject(prop);
    const hit = g.traceEnvironmentHit(-100, 0, 100, 0, 3);
...
test('damaged environment state is restored on regeneration', () => {
  ...
  const result = run(ctx, `(function () {
    ...
    tree.hp = 40;
    g.rememberEnvironmentState(tree);
    const stored = g.environmentState.get('t1');
    // Prune removes the live object but keeps remembered state.
    g.environmentObjects.delete('t1');
    const remembered = g.environmentState.has('t1') && stored.hp === 40;
    // Regeneration re-creates the object and restores remembered hp.
    const recreated = Object.assign({}, tree, { hp: tree.maxHp });
    g.addEnvironmentObject(recreated);
