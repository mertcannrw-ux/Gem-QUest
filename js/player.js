...
class Player {
  ...
  takeDamage(amount, sourceX, sourceY) {
    ...
    if (s.blockChance > 0 && runtimeRandom(this.game).chance(s.blockChance)) {
  ...
  update(dt, game) {
    ...
    // Combat behaviours (auto-attack, tempest fan, drone companions) are
    // delegated to the combat modules; the Player entity keeps progression,
    // state, movement, and damage handling.
    this.autoAttack(dt, game, s);
    this.updateDrones(dt, game, s);
  }
